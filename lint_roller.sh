...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive coverage_pyver_pragma/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive coverage_pyver_pragma/
  ...
  echo "Running flake8"
    ...
    >&2 flake8 coverage_pyver_pragma/
