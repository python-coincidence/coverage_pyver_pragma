...
`coverage_pyver_pragma` uses `tox` to automate testing, autoformatting and packaging.

For more information on installing `tox` see https://tox.readthedocs.io/en/latest/install.html

## Coding style

`coverage_pyver_pragma` uses `yapf` for code formatting and `isort` to sort imports.
